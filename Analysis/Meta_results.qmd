...
```{r}
plant_MR_mod1 <- rma.mv(Effect_size ~ Method + Animal, V = CVC_mat_plant_imp, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_comp_plant)

summary(plant_MR_mod1)

plant_MR_mod2 <- rma.mv(Effect_size ~ -1 + Method + Animal, V = CVC_mat_plant_imp, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_comp_plant) #suppressed intercept - need to fix so it works at all levels
