...
lscape_MR_pb4 <- rma.mv(Effect_size, V = VCV_mat_lscape, mods = ~ -1 + Method + Plot_size + Year + root_inv_ESS, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_comp_lscape_pb2)
...
for (i in seq_along(df_imp_plant_pb1)) {
  imp_plant_MMA_pb1[[i]] <- rma.mv(Effect_size, V = VCV_mat_plant_imp[[i]], mods = ~ 1 + Year + root_inv_ESS, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_imp_plant_pb1[[i]], verbose = TRUE) }
...
saveRDS(imp_plant_MMA_mod1_rob, here("Results", "Models", "Plant", "imp_Plant_MMA_rob.rds"))
...
imp_coupe_MMA_pb1 <- lapply(seq_along(df_imp_coupe_pb1), 
                              function(i) MMA_pb(df_imp_coupe_pb1[[i]], VCV_mat_coupe_imp[[i]], i))
summary(pool(imp_coupe_MMA_pb1))
saveRDS(imp_coupe_MMA_pb1, here("Results", "Models", "Coupe", "imp_coupe_MMA_pb.rds"))


imp_coupe_MMA_rob <- list()
for (i in seq_along(df_imp_coupe_pb1)) {
  imp_coupe_MMA_rob[[i]] <- robust(imp_coupe_MMA_pb1[[i]], cluster = df_imp_coupe_pb1[[i]]$Paper_ID) }

saveRDS(imp_coupe_MMA_pb_rob, here("Results", "Models", "Coupe", "imp_coupe_MMA_pb_rob.rds"))

rm(imp_coupe_MMA_pb1, imp_coupe_MMA_rob)
```
...
imp_lscape_MMA_pb1 <- mclapply(seq_along(df_imp_lscape_pb1), 
                               function(i) MMA_pb(df_imp_lscape_pb1[[i]], VCV_mat_lscape_imp[[i]], i), mc.cores = num_cores)
...
for (i in seq_along(df_imp_lscape_pb1)) {
  imp_lscape_MMA_pb_rob[[i]] <- robust(imp_lscape_MMA_pb1[[i]], cluster = df_imp_lscape_pb1[[i]]$Paper_ID) }

saveRDS(imp_lscape_MMA_pb_rob, here("Results", "Models", "Landscape", "imp_lscape_MMA_pb_rob.rds"))
rm(imp_lscape_MMA_pb1, imp_lscape_MMA_pb_rob)
```
