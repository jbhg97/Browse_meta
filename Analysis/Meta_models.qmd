...
```{r}
df_comp_plant2 <- df_comp_plant %>% mutate(Eff_SS = (4*(Con_N*Trt_N)) / (Con_N + Trt_N))
```

```{r}
funnel(df_comp_plant2$Effect_size, df_comp_plant2$Sampling_variance, ni = df_comp_plant2$Eff_SS, yaxis="ni",
...
df_comp_plant3 <- df_comp_plant2 %>% mutate(Year = Year - mean(Year),
                                            root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) )

df_comp_plant4 <- df_comp_plant1 %>% mutate(Year = Year - mean(Year),
                                            root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) )
```
...
plant_MR_pb5 <- rma.mv(Effect_size, V = VCV_mat_plant1, mods = ~ Months + Plot_size + Year + root_inv_ESS, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_comp_plant4)
...
#centre the publication year
#calculate sqrt inverse effective sample size
df_imp_plant3 <- lapply(df_imp_plant1, function(df){ 
  df %>% mutate(Year = Year - mean(Year), 
                root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) ) })

df_imp_plant4 <- lapply(df_imp_plant2, function(df){ 
  df %>% mutate(Year = Year - mean(Year), 
                root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) ) })
```
...
for (i in seq_along(df_imp_plant3)) {
  imp_plant_MMA_pb1[[i]] <- rma.mv(Effect_size, V = VCV_mat_plant_imp[[i]], mods = ~ 1 + Year + root_inv_ESS, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_imp_plant3[[i]], verbose = TRUE) }
...
for (i in seq_along(df_imp_plant3)) {
  imp_plant_MR_pb1[[i]] <- rma.mv(Effect_size, V = VCV_mat_plant_imp[[i]], mods = ~ Method + Plot_size + Year + root_inv_ESS, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_imp_plant3[[i]], verbose = TRUE) }
summary(pool(imp_plant_MR_pb1))

imp_plant_MR_pb3 <- list()
for (i in seq_along(df_imp_plant3)) {
  imp_plant_MR_pb3[[i]] <- rma.mv(Effect_size, V = VCV_mat_plant_imp[[i]], mods = ~ Animal + Plot_size + Year + root_inv_ESS, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_imp_plant3[[i]], verbose = TRUE) }
summary(pool(imp_plant_MR_pb3))

imp_plant_MR_pb5 <- list()
for (i in seq_along(df_imp_plant4)) {
  imp_plant_MR_pb5[[i]] <- rma.mv(Effect_size, V = VCV_mat_plant_imp1[[i]], mods = ~ Months + Plot_size + Year + root_inv_ESS, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_imp_plant4[[i]], verbose = TRUE) }
...
```{r}
rm(imp_plant_MMA_mod1, imp_plant_MMA_pb1, imp_plant_MR_mod1, imp_plant_MR_mod2, imp_plant_MR_mod3, imp_plant_MR_mod4, imp_plant_MR_mod5, imp_plant_MR_mod1_rob, imp_plant_MR_mod3_rob, imp_plant_MR_mod5_rob, imp_plant_MR_pb1, imp_plant_MR_pb3, imp_plant_MR_pb5, VCV_mat_plant_imp, df_imp_plant, df_imp_plant1, df_imp_plant2)
```
...
imp_coupe_MR_mod1 <- lapply(seq_along(df_imp_coupe2), 
                              function(i) MR1(df_imp_coupe2[[i]], VCV_mat_coupe_imp1[[i]], i))
...
imp_coupe_MR_mod2 <- lapply(seq_along(df_imp_coupe2), 
                              function(i) MR2(df_imp_coupe2[[i]], VCV_mat_coupe_imp1[[i]], i))
summary(pool(imp_coupe_MR_mod2))
saveRDS(imp_coupe_MR_mod2, here("Results", "Models", "Coupe", "imp_coupe_MR2.rds"))
rm(imp_coupe_MR_mod2)
```
...
df_imp_coupe3 <- lapply(df_imp_coupe1, function(df){ 
  df %>% mutate(Year = Year - mean(Year), 
                root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) ) })

df_imp_coupe4 <- lapply(df_imp_coupe2, function(df){ 
  df %>% mutate(Year = Year - mean(Year), 
                root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) ) })
```
...
imp_coupe_MMA_pb1 <- lapply(seq_along(df_imp_coupe3), 
                              function(i) MMA_pb(df_imp_coupe3[[i]], VCV_mat_coupe_imp[[i]], i))
summary(pool(imp_coupe_MMA_pb1))
saveRDS(imp_coupe_MMA_pb1, here("Results", "Models", "Coupe", "imp_coupe_MMA_pb1.rds"))
rm(imp_coupe_MMA_pb1)
```

```{r}
imp_coupe_MR_pb1 <- list()
imp_coupe_MR_pb1 <- mclapply(seq_along(df_imp_coupe4), 
                             function(i) MR1_pb(df_imp_coupe4[[i]], VCV_mat_coupe_imp1[[i]], i), mc.cores = num_cores)
...
imp_coupe_MR_pb3 <- mclapply(seq_along(df_imp_coupe3), 
                             function(i) MR3_pb(df_imp_coupe3[[i]], VCV_mat_coupe_imp[[i]], i), mc.cores = num_cores)
...
imp_coupe_MR_pb5 <- mclapply(seq_along(df_imp_coupe4), 
                             function(i) MR5_pb(df_imp_coupe4[[i]], VCV_mat_coupe_imp1[[i]], i), mc.cores = num_cores)
...
```{r}
rm(df_imp_coupe, df_imp_coupe1, df_imp_coupe2, df_imp_coupe3, df_imp_coupe4, VCV_mat_coupe_imp, VCV_mat_coupe_imp1)
```
...
imp_lscape_MMA_mod1 <- lapply(seq_along(df_imp_lscape1), 
                                function(i) MMA(df_imp_lscape1[[i]], VCV_mat_lscape_imp[[i]], i))

saveRDS(imp_lscape_MMA_mod1, here("Results", "Models", "Landscape", "imp_lscape_MMA.rds"))
rm(imp_lscape_MMA_mod1)
```
...
imp_lscape_MR_mod1 <- mclapply(seq_along(df_imp_lscape1), 
                               function(i) MR1(df_imp_lscape1[[i]], VCV_mat_lscape_imp[[i]], i), mc.cores = num_cores)
...
df_imp_lscape3 <- lapply(df_imp_lscape1, function(df){ 
  df %>% mutate(Year = Year - mean(Year), 
                root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) ) })

df_imp_lscape4 <- lapply(df_imp_lscape2, function(df){ 
  df %>% mutate(Year = Year - mean(Year), 
                root_inv_ESS = sqrt((Con_N + Trt_N)/(Con_N*Trt_N)) ) })
```
...
imp_lscape_MMA_pb1 <- mclapply(seq_along(df_imp_lscape3), 
                               function(i) MMA_pb(df_imp_lscape3[[i]], VCV_mat_lscape_imp[[i]], i), mc.cores = num_cores)
summary(pool(imp_lscape_MMA_pb1))
saveRDS(imp_lscape_MMA_pb1, here("Results", "Models", "Landscape", "imp_lscape_MMA_pb1.rds"))
rm(imp_lscape_MMA_pb1)
```

```{r}
imp_lscape_MR_pb1 <- list()
imp_lscape_MR_pb1 <- mclapply(seq_along(df_imp_lscape3), 
                                function(i) MR1_pb(df_imp_lscape3[[i]], VCV_mat_lscape_imp[[i]], i), mc.cores = num_cores)
summary(pool(imp_lscape_MR_pb1))
saveRDS(imp_lscape_MR_pb1, here("Results", "Models", "Landscape", "imp_lscape_MR_pb1.rds"))
rm(imp_lscape_MR_pb1)
```

```{r}
imp_lscape_MR_pb3 <- list()
imp_lscape_MR_pb3 <- mclapply(seq_along(df_imp_lscape3), 
                              function(i) MR3_pb(df_imp_lscape3[[i]], VCV_mat_lscape_imp[[i]], i), mc.cores = num_cores)
summary(pool(imp_lscape_MR_pb3))
saveRDS(imp_lscape_MR_pb3, here("Results", "Models", "Landscape", "imp_lscape_MR_pb3.rds"))
rm(imp_lscape_MR_pb3)
```

```{r}
imp_lscape_MR_pb5 <- list()
imp_lscape_MR_pb5 <- mclapply(seq_along(df_imp_lscape4), 
                              function(i) MR5_pb(df_imp_lscape4[[i]], VCV_mat_lscape_imp1[[i]], i), mc.cores = num_cores)
summary(pool(imp_lscape_MR_pb5))
saveRDS(imp_lscape_MR_pb5, here("Results", "Models", "Landscape", "imp_lscape_MR_pb5.rds"))
rm(imp_lscape_MR_pb5)
```
