...
Going to model all the effects together and make an orchard plot for presentation slides. Essentially trying to show what the general trend is but there's lots of variation so we need to tease that apart to see what actually works well - hence the rationale of splitting the scales and then into control methods/taxa.

```{r}
df_comp_all <- read_excel(here("Data processing", "Meta_full_data.xlsx"), sheet = "Complete case effects") # reading in from here rather than file in Analysis folder as for an unnknown reason read in from All_compcase_es.xlsx converts Count columns to Boolean.
```

```{r}
df_comp_all <- df_comp_all %>% mutate(OLRE = as.factor(row_number()))
```

```{r}
VCV_mat_all <- vcalc(vi = Sampling_variance, cluster = Shared_data, data = df_comp_all, rho = 0.5, nearpd = TRUE)
```

```{r}
all_MMA <- rma.mv(Effect_size ~ 1, V = VCV_mat_all, test = "t", dfs = "contain", random = list(~1|Paper_ID, ~1|Repeat_measures, ~1|Plant_genus, ~1|OLRE), data = df_comp_all)
```

```{r}
all_orchard <- orchard_plot(all_MMA, group = "Paper_ID", xlab = "Effect size / SMD") + ylim(c(-5, 10))
```
...
orchaRd::cv_ml(plant_MMA_mod1)

orchaRd::m1_ml(plant_MMA_mod1)
...
```{r}
orchard_plot(plant_MMA_mod1, group = "Paper_ID", xlab = "Effect size / SMD")
mod_results(plant_MMA_mod1, group = "Paper_ID")
```
...
```{r}
plant_funnel <- funnel(df_comp_plant_pb$Effect_size, df_comp_plant_pb$Sampling_variance, ni = df_comp_plant_pb$Eff_SS, yaxis="ni",
...
orchaRd::cv_ml(coupe_MMA_mod1)

orchaRd::m1_ml(coupe_MMA_mod1)
...
```{r}
orchard_plot(coupe_MMA_mod1, group = "Paper_ID", xlab = "Effect size / SMD") + ylim(-5,18)
mod_results(coupe_MMA_mod1, group = "Paper_ID")
```
...
```{r}
coupe_funnel <- funnel(df_comp_coupe_pb$Effect_size, df_comp_coupe_pb$Sampling_variance, ni = df_comp_coupe_pb$Eff_SS, yaxis="ni",
       ylab = "Effective sample size",
       xlab = "Effect size / SMD",
       back = "white",
       hlines = "black") 
...
orchaRd::cv_ml(lscape_MMA_mod1)

orchaRd::m1_ml(lscape_MMA_mod1)
...
```{r}
orchard_plot(lscape_MMA_mod1, group = "Paper_ID", xlab = "Effect size / SMD") + ylim(-5,10)
mod_results(lscape_MMA_mod1, group = "Paper_ID")
```
...
```{r}
lscape_funnel <- funnel(df_comp_lscape_pb$Effect_size, df_comp_lscape_pb$Sampling_variance, ni = df_comp_lscape_pb$Eff_SS, yaxis="ni",
       ylab = "Effective sample size",
       xlab = "Effect size / SMD",
       back = "white",
       hlines = "black")
...
```{r}
rm(imp_plant_MMA_mod1, imp_plant_MMA_mod1_rob, imp_plant_MR_mod1, imp_plant_MR_mod2, imp_plant_MR_mod3, imp_plant_MR_mod4, imp_plant_MR_mod5, imp_plant_MR_mod1_rob, imp_plant_MR_mod2_rob, imp_plant_MR_mod3_rob, imp_plant_MR_mod4_rob, imp_plant_MR_mod5_rob, VCV_mat_plant_imp, df_imp_plant, df_imp_plant1, df_imp_plant2)
```
