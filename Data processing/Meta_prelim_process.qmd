...
```{r}
df_lo2 <- df_lo1 %>% 
  select(Paper_ID, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_mean, Trt_N, Trt_mean, Trans)
```
...
df_lo3 <- df_lo2 %>% 
  ...
  select(Paper_ID, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_count, Trt_N, Trt_count, Trans)
```
...
saveRDS(df_imp_all1, "/homevol/jbhg/Browse_meta/Data processing/All_impcase_es")
```
