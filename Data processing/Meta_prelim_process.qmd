...
```{r}
df <- read_excel(here("Data processing", "Meta_full_data.xlsx"), sheet = "Effects (post responses)")
```
...
I need to attach publication year to all effects for publication bias tests.

```{r}
pub_yr <- read_excel(here("Data processing", "Meta_full_data.xlsx"), sheet = "Data extraction")
pub_yr1 <- pub_yr %>% mutate(ref = paste(Authors, `Article Title`, `Source Title`)) %>% select(ref, Year)

df1 <- left_join(df1, pub_yr1, by = "ref")
```
...
Split out Least squares means here into their own dataframe - excluding these from further analyses.
...
group_method <- function(Method){
  case_when(Method %in% c("Cover crop", "Plant refuge", "Olfactory misinformation") ~ "Neighbour effects",
...
Proportion style data measured across multiple plots and therefore producing a mean can be problematic. Need to process this so that first, the units are the same ie all proportions. Second, transform it to put the data on to an unbounded scale - see Harrison et al 2022 biorev paper.
...
```{r}
df_SMD6 <- df_SMD5 %>% 
  select(Paper_ID, Country, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_mean, Con_SD, Trt_N, Trt_mean, Trt_SD, Trans, Con_trans_mean, Trt_trans_mean, Year)
```
...
```{r}
write_xlsx(df_SMD_sdtrans1, here("Data processing", "SMD_compcase_raw and transform.xlsx"))
```
...
I need to assess the quality of the data that will be used for the imputation and further downstream analyses. Instances where the control/treatment SD is 0 will not be suitable for generating SMD effect sizes as they cause an effect size of infinity. Moreover, instances where either control or treatment sample size is 1 will result in an SD = 0 creating the same problem. I will remove these cases.
...
```{r}
df_lo2 <- df_lo1 %>% 
  select(Paper_ID, Country, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_mean, Trt_N, Trt_mean, Trans, Year)
```
...
df_lo3 <- df_lo2 %>% 
  ...
  select(Paper_ID, Country, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_count, Trt_N, Trt_count, Trans, Year)
```
...
df_SMD_all1 <- df_SMD_all %>% mutate(Con_mean = if_else(Trans == "N", Con_mean, Con_trans_mean),
  ...
  select(Paper_ID, Country, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_mean, Con_SD, Trt_N, Trt_mean, Trt_SD, Trans, Year)
```
...
predmatHg[, c(2:5,9:15,22:26)] <- 0
predmatHg[c(1:17,19,20,22:26), ] <- 0 # these rows deal with SD imputation - using PaperID, plant genus, scale, plot size, time and N, mean and SD 
...
```{r}
write_xlsx(df_comp_SMD2, here("Data processing", "SMD_compcase_es.xlsx"))
```
...
```{r}
write_xlsx(df_comp_lo2, here("Data processing", "LO_compcase_es.xlsx"))
```
...
```{r}
saveRDS(df_imp_SMD2, here("Data processing", "SMD_impcase_es.rds"))
```
...
```{r}
saveRDS(df_imp_lo2, here("Data processing", "LO_impcase_es.rds"))
```
...
```{r}
write_xlsx(df_comp_all, here("Analysis", "All_compcase_es.xlsx")) # add to meta_full_data - Complete case effects sheet
saveRDS(df_imp_all1, here("Analysis", "All_impcase_es.rds")) # add iteration of this to meta_full_data - Imputed case effects sheet
```
