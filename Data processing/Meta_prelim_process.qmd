...
group_fauna <- function(Animal){
  case_when(Animal %in% c("Beaver", "Coypu", "Kangaroo", "Pademelon (captive)", "Wallaby", "Wallaby, pademelon, possum", "Hares", "Possum (captive)", "Hyrax", "Kangaroo, wallaby", "Rabbits", "Voles") ~ "Other",
            Animal %in% c("Deer, ELk", "Deer, sheep", "Goats", "Moose", "Reindeer", "Sheep", "Wild boar, sheep, goat", "Cattle", "Deer, elk", "Deer, pigs", "Elk", "Ungulates") ~ "Other ungulates",
            Animal == "Deer" ~ "Deer",
...
group_method <- function(Method){
  case_when(Method %in% c("Cover crop", "Plant refuge") ~ "Plant refuge",
...
predmatHg[, c(2:13,20,21)] <- 0
predmatHg[c(1:15,17,18,20,21), ] <- 0
...
```{r}
saveRDS(df_imp_SMD2, "/homevol/jbhg/Browse_meta/Data processing/SMD_imp_data_log")
