...
Does Geary's test perform better on the log data than the raw data? - almost identical to the non-log data
...
mice::xyplot(impData_SMD, Trt_SD ~ Trt_mean)
...
```{r}
# Next step of imputation goes here... back transformation of imputed log data to gain imputed raw data for effect size generation.
```
...
Conventional SMD effects - NEED TO EDIT CODE FROM HERE IN TO ACT ON ALL IMPUTED DATASETS (use with() calls)

```{r}
df_SMD_all3 <- df_SMD_all2 %>% 
  mutate(Effect_size = escalc("SMD", m1i = Trt_mean, sd1i = Trt_SD, n1i = Trt_N, m2i = Con_mean,
...
```{r}
df_SMD_all4 <- df_SMD_all3 %>% unnest_wider(Effect_size) %>% 
