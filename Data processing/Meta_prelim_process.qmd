...
predmatHg[, c(2:4,9:14,21:24)] <- 0
predmatHg[c(1:16,18,19,21:24), ] <- 0 # these rows deal with SD imputation - using PaperID, plant genus, scale, plot size, time and N, mean and SD 
predmatHg[7, c(1,6,15,18)] <- 1 # this imputes plot size using PaperID, scale and N (smaller plots = higher N and vice versa, Spake et al. (2020))
```
...
# need a plot that shows the imputed plot size data
...
```{r}
saveRDS(df_imp_SMD2, "/homevol/jbhg/Browse_meta/Data processing/SMD_impcase_es")
```
...
```{r}
saveRDS(df_imp_lo2, "/homevol/jbhg/Browse_meta/Data processing/LO_impcase_es")
```
...
```{r}
write_xlsx(df_comp_all, "/homevol/jbhg/Browse_meta/Data processing/All_compcase_es.xlsx")
saveRDS(df_imp_all, "/homevol/jbhg/Browse_meta/Data processing/All_impcase_es")
```
...
write_xlsx(df_comp_plant, "/homevol/jbhg/Browse_meta/Analysis/Comp_all_plant.xlsx")
  
df_comp_coupe <- df_comp_all %>% filter(Scale == "Coupe")
write_xlsx(df_comp_coupe, "/homevol/jbhg/Browse_meta/Analysis/Comp_all_coupe.xlsx")
  
df_comp_lscape <- df_comp_all %>% filter(Scale == "Landscape")
write_xlsx(df_comp_lscape, "/homevol/jbhg/Browse_meta/Analysis/Comp_all_lscape.xlsx")
```

```{r}
df_imp_plant <- lapply(df_imp_all, function(df){ df %>% filter(Scale == "Plant") })
saveRDS(df_imp_plant, "/homevol/jbhg/Browse_meta/Analysis/Imp_all_plant")
  
df_imp_coupe <- lapply(df_imp_all, function(df){ df %>% filter(Scale == "Coupe") })
saveRDS(df_imp_coupe, "/homevol/jbhg/Browse_meta/Analysis/Imp_all_coupe")
  
df_imp_lscape <- lapply(df_imp_all, function(df){ df %>% filter(Scale == "Landscape") })
saveRDS(df_imp_lscape, "/homevol/jbhg/Browse_meta/Analysis/Imp_all_lscape")
```
