...
```{r}
df_SMD6 <- df_SMD5 %>% 
  select(Paper_ID, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_mean, Con_SD, Trt_N, Trt_mean, Trt_SD, Trans, Con_trans_mean, Trt_trans_mean)
```
...
df_SMD_all1 <- df_SMD_all %>% mutate(Con_mean = if_else(is.na(Trans), Con_mean, Con_trans_mean),
                              ...
                              Trt_mean = if_else(is.na(Trans), Trt_mean, Trt_trans_mean),
                              Trt_SD = if_else(is.na(Trans), Trt_SD, Trt_trans_SD)) %>% 
  select(Paper_ID, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_mean, Con_SD, Trt_N, Trt_mean, Trt_SD, Trans)
```
...
```{r}
df_SMD_all4 <- df_SMD_all3 %>% 
  filter(Con_mean != -Inf | is.na(Con_mean)) %>% filter(Trt_mean != Inf | is.na(Trt_mean))
```

#### Build the predictor matrix

Due to the negative values created by the logit transformation we will pursue imputation using the raw data rather than log transformed. Therefore, these columns can be removed before building the predictor matrix.

```{r}
df_SMD_all5 <- df_SMD_all4 %>% dplyr::select(!c(log_Con_mean, log_Con_SD, log_Trt_mean, log_Trt_SD))
```
...
predmatHg <- make.predictorMatrix(df_SMD_all5)
predmatHg[, c(2:5,10:14,21,22)] <- 0
predmatHg[c(1:16,18,19,21,22), ] <- 0
```
...
impData_SMD <- mice(df_SMD_all5, m=20, maxit=50, meth = c("","","","","","","","","","","","","","","","","pmm","","","pmm","",""), pred = predmatHg, seed=500)
summary(impData_SMD)
```
...
df_imp_SMD <- complete(impData_SMD, "all")
...
```{r}
saveRDS(df_imp_SMD2, "/homevol/jbhg/Browse_meta/Data processing/SMD_imp_data")
...
```{r}
df_lo2 <- df_lo1 %>% 
  select(Paper_ID, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_mean, Trt_N, Trt_mean)
```
...
df_lo3 <- df_lo2 %>% 
  ...
  select(Paper_ID, Method, Type, Animal, Plant_genus, Scale, Plot_size, Months, Variable, Units, Confounding, Effect_type, Shared_data, Repeat_measures, Con_N, Con_count, Trt_N, Trt_count)
```
...
```{r}
df_lo_conv <- df_lo4 %>% mutate(Effect_size = lo_to_SMD_ES(Effect_size), 
...
df_imp_all <- lapply(df_imp_SMD2, function(df){
  bind_rows(df, df_lo_conv)
 })
...
```{r}
df_imp_plant <- lapply(df_imp_all, function(df){
  df %>% filter(Scale == "Plant") })
saveRDS(df_imp_plant, "/homevol/jbhg/Browse_meta/Analysis/Imp_comb_plant")
  
df_imp_coupe <- lapply(df_imp_all, function(df){
  df %>% filter(Scale == "Coupe") })
saveRDS(df_imp_coupe, "/homevol/jbhg/Browse_meta/Analysis/Imp_comb_coupe")
  
  
df_imp_lscape <- lapply(df_imp_all, function(df){
  df %>% filter(Scale == "Landscape") })
saveRDS(df_imp_lscape, "/homevol/jbhg/Browse_meta/Analysis/Imp_comb_lscape")
