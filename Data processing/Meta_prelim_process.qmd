...
### Imputing errors and Convert errors to SD
...
```{r}
#Error imputation code will go here
```

(Don't worry about losing effects at this stage - data set is incomplete in some areas which is causing NAs to be dropped)
...
Might need to account for the LS means here before the error conversion.
...
to_SD <- function(Error_type, Value, Sample_size, Mean) {
  case_when(
    ...
    Error_type == "95CI" ~ Value #issue here as the only paper that I have errors as 95CI also used LS means - ask Dan how to clear this up
  )
}
```
...
Use metafor package escalc() to generate the effect sizes. Will then need to review the variables that were measured in order to make sure that instances where a negative impact on the variable is a positive browsing outcome are accounted for. Also need to investigate how to convert the F statistic data from Andelt et al into an SMD effect.

Conventional SMD effects

```{r}
df_SMD7 <- df_SMD6 %>% 
  mutate(Effect_size = escalc("SMD", m1i = Trt_mean, sd1i = Trt_SD, n1i = Trt_N, m2i = Con_mean, sd2i = Con_SD, n2i = Con_N))
```

F stat SMD effects

```{r}
# write relevant f stat code here
```

Check on the variables that are in play and then write a function to swap relevant effects.
...
```{r}
sign_switch_SMD <- function(Variable, Value) {
  case_when(Variable ==  ~ Value * -1, # insert list of variables to be switched here
            TRUE  ~ Value)
}
```
...
```{r}
df_SMD8 <- df_SMD7 %>% 
  mutate(Effect_size = sign_switch_SMD(Variable, Effect_size))
```
...
df_lo1 <- df_lo %>% 
  rename(Method = `Control method`, Type = `Method sub-type`, Scale = `Scale (plant, coupe, landscape)`, Variable = `Var. measured`,
         Con_N = `Control sample size`, Con_ = `Control mean`, Trt_N = `Treatment sample`, Trt_mean = `Treatment mean`)
```

```{r}
df_lo2 <- df_lo1 %>% 
  select(Paper_ID, Method, Type, Animal, `Plant genus`, Scale, Months, Variable, Units, `Confounding factors`, `Effect type`, Con_N,
         Con_mean, Trt_N, Trt_mean)
```
...
```{r}
df_lo4 <- df_lo3 %>% 
  mutate(Effect_size = escalc(measure = "OR", ai = Trt_count, n1i = Trt_N, ci = Con_count, n2i = Con_N))
```
...
```{r}
sign_switch_lo <- function(Variable, Value) {
  case_when(Variable == "Mortality" | "Plants damaged" | "Plants browsed" | "Leader browsing" | "Lateral browsing" | 
                        "Bud damage" | "Leaf damage" | "Stems harvested" ~ Value * -1,
            TRUE ~ Value)
...
```{r}
df_lo5 <- df_lo4 %>% 
  mutate(Effect_size = sign_switch_lo(Variable, Effect_size))
```

Write out the file with the generated effects ready for analysis

```{r}
write_xlsx(df_lo5, "Meta_lo_es.xlsx") #add metadata sheet to this file
```

Add code for converting log odds effects to something comparable to SMD - join all the effects into one dataframe...
