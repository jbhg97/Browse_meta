...
```{r}
pub_yr <- read_excel(here("Data processing", "Lit review data.xlsx"), sheet = "Complete list")
...
```{r}
set.seed(123) #set random seed for imputation for reproducibility
...
```{r}
write_xlsx(df_comp_all, here("Data processing", "All_compcase_es.xlsx"))
saveRDS(df_imp_all1, here("Data processing", "All_impcase_es.rds"))
```
...
write_xlsx(df_comp_plant, here("Analysis", "Comp_all_plant.xlsx"))
  
df_comp_coupe <- df_comp_all %>% filter(Scale == "Coupe")
write_xlsx(df_comp_coupe, here("Analysis", "Comp_all_coupe.xlsx"))
  
df_comp_lscape <- df_comp_all %>% filter(Scale == "Landscape")
write_xlsx(df_comp_lscape, here("Analysis", "Comp_all_lscape.xlsx"))
```

```{r}
df_imp_plant <- lapply(df_imp_all1, function(df){ df %>% filter(Scale == "Plant") })
saveRDS(df_imp_plant, here("Analysis", "Imp_all_plant.rds"))
  
df_imp_coupe <- lapply(df_imp_all1, function(df){ df %>% filter(Scale == "Coupe") })
saveRDS(df_imp_coupe, here("Analysis", "Imp_all_coupe.rds"))
  
df_imp_lscape <- lapply(df_imp_all1, function(df){ df %>% filter(Scale == "Landscape") })
saveRDS(df_imp_lscape, here("Analysis", "Imp_all_lscape.rds"))
```
