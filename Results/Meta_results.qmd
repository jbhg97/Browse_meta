...
plant_month_bubble <- function(df, model, method, colour, xmin, xmax, ymin, ymax, k, n){
  ...
  plot <- ggplot(data = df1, aes(x = Months, y = Effect_size)) + 
    ...
    labs(x = "Time / months", y = "Effect size / SMD", size = "Precision (1/SE)", title = paste0(method,", k = ", k, "(",n,")")) + 
...
month_bubble <- function(df, model, method, colour, xmin, xmax, ymin, ymax, k, n){
...
map <- ggplot() +
  ...
  scale_fill_gradientn(colors = c("lightblue","lightgreen","gold"), values = scales::rescale(c(1, 10, max(world$N, na.rm = TRUE))), name = "N") +
  theme_bw() + 
...
save_plot(map_plot, "meta_map.png", 7.2)
```
...
p3 <- plant_month_bubble(df_comp_plant, plant_MR_mod5, method = "Repellents", colour = "#CC6677", xmin = 0, xmax = 1.25, ymin = -4, ymax = 6, k = 121, n = 10)
...
c3 <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Exclusion fence", "#88CCEE", 0, 140, -5, 10, 219, 20)
c3_sup <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Exclusion fence", "#88CCEE", 0, 140, -20, 17, 219, 20)
c4 <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Repellents", "#332288", 0, 50, -4, 6, 415, 16)
c5 <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Tree guards", "#009E73", 0, 65, -5, 10, 339, 6)
c6 <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Other fencing", "#117733", 0, 30, -5, 5, 15, 3)
c7 <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Natural debris", "#CC6677", 0, 65, -3, 5, 23, 2)
c8 <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Stock manipulation", "#AA4499", 0, 65, -3, 4, 111, 3)
c9 <- month_bubble(df_comp_coupe, coupe_MR_mod5, "Neighbour effects", "#DDCC77", 0, 50, -4, 3, 24, 1)
...
l3 <- month_bubble(df_comp_lscape, lscape_MR_mod5, "Exclusion fence", "#88CCEE", 0, 250, -2, 9, 413, 36)
l4 <- month_bubble(df_comp_lscape, lscape_MR_mod5, "Repellents", "#332288", 0, 65, -2, 4, 73, 8)
l5 <- month_bubble(df_comp_lscape, lscape_MR_mod5, "Tree guards", "#44AA99", 0, 65, -2, 4, 56, 7)
l6 <- month_bubble(df_comp_lscape, lscape_MR_mod5, "Natural debris", "#CC6677", 0, 20, -3, 3.5, 15, 2)
