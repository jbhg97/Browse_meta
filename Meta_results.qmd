...
df_SMD <- read_excel("Meta_SMD_es", sheet = "SMD")
df_lo <- read_excel("Meta_lo_es", sheet = "Log odds")

df_SMD1 <- df_SMD %>% select(Paper_ID, Method, Type, Animal, `PLant genus`, Scale, `Effect type`, Effect_size)
df_lo1 <- df_lo %>% select(Paper_ID, Method, Type, Animal, `PLant genus`, Scale, `Effect type`, Effect_size)

df_table <- rbind(df_SMD1, df_lo1)
